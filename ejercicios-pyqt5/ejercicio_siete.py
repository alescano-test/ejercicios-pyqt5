...
#!------------------------------------------------------------------------------------------------------
    ...
    def al_clic_agregar(self):
        # * Cantidad de items en el combo box.
        cant_items = self.opciones.count()
        # * El indice del indice en el que estamos posicionados.
        indice = self.opciones.currentIndex()
        # * texto = string que el usuario ingresa en el campo de entrada, si hace clic en el botón "Aceptar"
        texto, okPressed = QInputDialog.getText(
            self, "Agregar item", "Nombre: ")
        ...
        if okPressed and texto != '':
            ...
            if cant_items == 0:
                # * Agrega el texto en el indice que se está seleccionando.
                self.opciones.insertItem(indice, texto)
            ...
            # ! Crea popUp notificando agregado exitoso.
            msg = QMessageBox(QMessageBox.Information,
                              "Agregar item", "El item se ha agregado!")
            # ! Se ejecuta la notificación, esto devuelve el boton que se oprime.
            msg.exec()
        ...
        if okPressed and texto == '':
            ...
            msg = QMessageBox(QMessageBox.Warning, "Error",
                              "El campo no puede estar vacío", QMessageBox.Yes)
            # ! Se ejecuta la notificación, esto devuelve el boton que se oprime.
            msg.exec()
...
#!------------------------------------------------------------------------------------------------------
    ...
    def al_clic_editar(self):
        ...
        indice = self.opciones.currentIndex()
        texto, okPressed = QInputDialog.getText(
            self, "Editar item", "Ingresa nuevo nombre: ")

        if okPressed and texto != '':
            msg = QMessageBox(QMessageBox.Warning, "Editar item",
                              "¿Seguro quieres editar el item?", QMessageBox. Yes | QMessageBox.No)
            resultado = msg.exec()
            if resultado == QMessageBox.Yes:
                self.opciones.removeItem(indice)
                self.opciones.addItem(texto)
                msg = QMessageBox(QMessageBox.Warning,
                                  "Editar item", "Item editado.")
        ...
        if okPressed and texto == '':
            msg = QMessageBox(QMessageBox.Critical, "Editar item",
...
#!------------------------------------------------------------------------------------------------------
    ...
    def al_clic_quitar(self):
        indice = self.opciones.currentIndex()
        msg = QMessageBox(QMessageBox.Warning, "Eliminar item",
                          "¿Seguro quieres eliminar el item?", QMessageBox. Yes | QMessageBox.No)
        resultado = msg.exec()
        if resultado == QMessageBox.Yes:
            self.opciones.removeItem(indice)
            msg = QMessageBox(QMessageBox.Warning, "Eliminar item",
                              "Item eliminado.", QMessageBox. Yes)
            msg.exec()
        else:
            pass
...
#!------------------------------------------------------------------------------------------------------
    ...
    def al_clic_quitar_todos(self):
        msg = QMessageBox(QMessageBox.Warning, "Eliminar item",
                          "¿Seguro quieres eliminar todos los items?", QMessageBox. Yes | QMessageBox.No)
        resultado = msg.exec()
        if resultado == QMessageBox.Yes:
            self.opciones.clear()
            msg = QMessageBox(QMessageBox.Warning, "Eliminar item",
                              "Eliminaste todos los items.", QMessageBox. Yes)
        else:
            pass
#!------------------------------------------------------------------------------------------------------
