...
class MiVentana(QMainWindow):
    ...
    def agregar_persona(self):
        ...
        if len(self.apellido.text()) == 0:
            msg = QMessageBox(QMessageBox.Warning, "Error",
                              "Tienes que completar el apellido")
            msg.exec()
        if len(self.email.text()) == 0:
            msg = QMessageBox(QMessageBox.Warning, "Error",
                              "Tienes que completar el email")
            msg.exec()
        if len(self.nombre.text()) and len(self.apellido.text()) and len(self.email.text()) != 0:
            ...
            self.tablaDatos.setItem(
                0, 1, QTableWidgetItem(self.apellido.text()))
...
#!-----------------------------------------------------------------------------------------------------
    def eliminar_persona(self):
        msg = QMessageBox(QMessageBox.Warning, "Eliminar",
                          "Seguro quieres eliminarlo?", QMessageBox.Yes | QMessageBox.No)
        resultado = msg.exec()
